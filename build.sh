...
#build for linux dev
scons platform=linuxbsd dev_build=yes target=template_debug tools=no use_lto=yes module_websocket_enable=no module_upnp_enable=no module_webrtc_enable=no module_openxr_enabled=no -j12